...
import { encrypt } from "./tdh2";
import { abi as IFunctionsRouterABI } from "../artifacts/@chainlink/contracts/src/v0.8/functions/dev/v1_0_0/interfaces/IFunctionsRouter.sol/IFunctionsRouter.json";
import { abi as IFunctionsCoordinatorABI } from "../artifacts/@chainlink/contracts/src/v0.8/functions/dev/v1_0_0/interfaces/IFunctionsCoordinator.sol/IFunctionsCoordinator.json";
...
export class SecretsManager {
  ...
  private functionsRouter: GetContractReturnType<
    IFunctionsRouter$Type["abi"],
    PublicClient<Transport, Chain>,
    WalletClient<Transport, Chain, Account>
  >;
  private functionsCoordinator!: GetContractReturnType<
    IFunctionsCoordinator$Type["abi"],
    PublicClient<Transport, Chain>,
    WalletClient<Transport, Chain, Account>
  >;
  ...
  constructor({
    ...
    functionsRouterAddress: `0x${string}`;
    donId: string;
  }) {
    this.walletClient = walletClient;
    this.donId = donId;

    this.functionsRouter = getContract({
      abi: IFunctionsRouterABI,
  ...
  public async initialize(): Promise<void> {
    ...
    this.functionsCoordinator = getContract({
      abi: IFunctionsCoordinatorABI,
      address: functionsCoordinatorAddress,
      walletClient: this.walletClient,
      publicClient: this.walletClient,
    }) as unknown as GetContractReturnType<
      IFunctionsCoordinator$Type["abi"],
      PublicClient<Transport, Chain>,
      WalletClient<Transport, Chain, Account>
  ...
  public async encryptSecretsUrls(
    ...
    try {
      ...
      throw Error(
        `Error encountered when attempting to validate a secrets URL: ${e}`
      );
    }
  ...
  public async verifyOffchainSecrets(secretsUrls: string[]): Promise<boolean> {
    ...
    for (const url of secretsUrls) {
      ...
      try {
        ...
        throw Error(
          `Error encountered when attempting to fetch URL ${url}: ${e}`
        );
      }

      if (!response.data?.encryptedSecrets) {
        throw Error(
          `URL ${url} did not return a JSON object with an encryptedSecrets field`
        );
      }

      if (!isHex(response.data.encryptedSecrets)) {
        throw Error(
          `URL ${url} did not return a valid hex string for the encryptedSecrets field`
        );
      }
      ...
      if (
        ...
        throw Error(
          `URL ${url} returned a different encryptedSecrets field than the previous URL`
        );
      }
  ...
  public async encryptSecrets(
    ...
    const encryptedSecretsHexstring =
      ...
      Buffer.from(
        encrypt(
          thresholdPublicKey,
          Buffer.from(JSON.stringify(donKeyEncryptedSecrets))
        )
      ).toString("hex");
  ...
  public async uploadEncryptedSecretsToDON({
    ...
    if (!isHex(encryptedSecretsHexstring)) {
    ...
    const encryptedSecretsBase64 = Buffer.from(
      encryptedSecretsHexstring.slice(2),
      "hex"
    ).toString("base64");
    const signerAddress = this.walletClient.account.address;
    const signerAddressBase64 = Buffer.from(
    ...
    const storageSignature = await this.walletClient.signMessage({
      message: JSON.stringify(message),
    });
    const storageSignatureBase64 = Buffer.from(
      storageSignature.slice(2),
      "hex"
    ).toString("base64");
    ...
    for (const url of gatewayRpcMessageConfig.gatewayUrls) {
      ...
      try {
        ...
        if (!response.data?.result?.body?.payload?.success) {
          throw Error(
            `Gateway response indicated failure:\n${JSON.stringify(
              response.data
            )}`
          );
        }
  ...
  private async createGatewayMessage({
    ...
    const gatewaySignature = await this.walletClient.signMessage({
      message: this.createGatewayMessageBody(body).toString(),
    });
  ...
  private extractNodeResponses(gatewayResponse: AxiosResponse): NodeResponse[] {
    ...
    if (
      gatewayResponse.data?.result?.body?.payload?.node_responses.length < 1
    ) {
  ...
  private verifyDONHostedSecrets(gatewayResponses: GatewayResponse[]): void {
    ...
    for (const nodeResponse of nodeResponses) {
      if (!nodeResponse.rows || nodeResponse.rows.length !== rows!.length) {
        throw Error(
          "One or more nodes responded with a different number of secrets entries"
        );
      }
      for (let i = 0; i < rows!.length; i++) {
        if (
          ...
          throw Error(
            "One or more nodes responded with different secrets entries"
          );
        }
      }
    }
